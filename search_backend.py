...
class SearchBackend:
    ...
    def upload_files(self, files, project_name, project_code, project_date):
        """处理文件上传"""
        try:
            logger.info("=== 开始文件上传调试 ===")
            logger.info(f"项目名称: {project_name}")
            logger.info(f"项目编号: {project_code}")
            logger.info(f"项目日期: {project_date} (类型: {type(project_date)})")
            
            # 验证日期
            if project_date:
                logger.info(f"日期值: '{project_date}'")
                try:
                    parsed_date = datetime.strptime(project_date, "%Y-%m-%d")
                    logger.info(f"日期解析成功: {parsed_date}")
            ...
            # 插入文档到向量库 - 确保传递了 project_date
            inserted_count = self.search_system.insert_documents(
                project_dir,
                project_name=project_name,
                project_date=project_date,  # 确保传递日期
            ...
            return {
                ...
                "project_name": project_name,
                "project_code": project_code,
                "project_date": project_date,  # 返回日期用于调试
                "files": saved_files
            }
    ...
    def get_projects(self):
        """直接从Zilliz获取所有项目及其统计信息"""
        try:
            # 直接从Zilliz获取所有项目名称
            all_projects = self.search_system.get_all_projects()
            logger.info(f"从Zilliz获取到的项目列表: {all_projects}")
            
            # 为每个项目获取详细信息
            projects_with_stats = []
            for project_name in all_projects:
                # 获取项目元数据
                metadata = self.search_system.get_project_metadata(project_name) or {}
                logger.info(f"项目 {project_name} 的元数据: {metadata}")
                
                # 获取文档数量
                doc_count = 0
                try:
                    expr = f"project_name == '{project_name}'"
                    count_result = self.search_system.collection.query(expr=expr, count_only=True)
                    
                    if isinstance(count_result, int):
                        doc_count = count_result
                    elif hasattr(count_result, '__len__'):
                        doc_count = len(count_result)
                    else:
                        try:
                            doc_count = int(count_result)
                        except (TypeError, ValueError):
                            doc_count = 0
                except Exception as e:
                    logger.warning(f"获取项目 {project_name} 文档数量失败: {str(e)}")
                    doc_count = 0
                ...
                projects_with_stats.append({
                    ...
                    "project_date": metadata.get('project_date', '未知日期')
                })
            
            # 按项目名称排序
            projects_with_stats.sort(key=lambda x: x["name"].lower())
