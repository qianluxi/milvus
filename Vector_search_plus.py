...
# from langchain_community.llms import ModelScopeLLM
...
# 初始化 Hugging Face 客户端（全局用这个）,但modelscope改为用config.py
# client = OpenAI(
...
# 模型配置 - 取消huggingface配置导入，改为modelscope配置导入，并重建ModelScopeLLM类
from openai import OpenAI

class ModelScopeLLM:
    """OpenAI 兼容封装的 ModelScope LLM API"""
    ...
    def generate(self, prompt, **kwargs):
        """
        支持在调用时传入 temperature、max_tokens 等参数
        用法:
            llm.generate("你好", temperature=0.7, max_tokens=512)
        """
        # 允许外部通过 kwargs 覆盖默认值
        temperature = kwargs.get("temperature", 0.7)      # 默认 0.7
        max_tokens  = kwargs.get("max_tokens", 512)       # 默认 512
...
class VectorSearchSystem:
    def __init__(self, collection_name="text_searchMC", max_retries=5, zilliz_config=None, model_config=None):
        ...
        self.embeddings = ModelScopeEmbeddings(
            model_id=MODEL_CONFIG["embedding"]["model_id"]   # 从 config.py 读取
        )
        ...
        self.llm = ModelScopeLLM(
            ...
            api_key=HF_TOKEN,
            #task="conversational",   # ✅ 显式指定对话任务,这是huggingface专用
            #temperature=0.7,       # 这是huggingface专用格式
            #max_new_tokens=512,    # 这是huggingface专用格式
    ...
    def update_context(self, constraint_type: str, value: str):
        """
        添加或更新筛选约束（追加逻辑，不覆盖）。
        constraint_type: "project_name", "project_code", "project_date" 等
        value: 字符串形式的过滤值
        """
        if not constraint_type or value is None:
            logger.warning("update_context: 约束类型或值为空，忽略")
            return False

        constraints = self.context_manager.setdefault("constraints", [])
        new_c = {"type": constraint_type, "value": value}

        # 避免重复添加完全相同的约束
        if new_c not in constraints:
            constraints.append(new_c)

        # 每次更新后重新计算匹配的项目集合
        project_set = self._compute_project_set_for_constraints()
        self.context_manager["current_project_set"] = project_set
        logger.info(f"[上下文更新] 新增约束: {new_c} -> 当前匹配项目数: {len(project_set)}")
        return True
    
    def _compute_project_set_for_constraints(self):
        """
        根据 context_manager['constraints'] 计算匹配的项目集合（并集逻辑）。
        支持按项目名、项目编号、日期模糊匹配。
        """
        constraints = self.context_manager.get("constraints", []) or []
        if not constraints:
            return []

        # 一次性加载全部项目名
        all_projects = set(self.get_all_projects() or [])
        matched = set()

        # 从 Milvus 获取项目日期字典
        try:
            project_dates = self._get_project_dates_from_milvus(all_projects) if all_projects else {}
        except Exception:
            project_dates = {}
        ...
        for c in constraints:
            ...
            if ctype == "project_name":
                # 模糊匹配项目名（包含）
                for pname in all_projects:
                    if cval.lower() in (pname or "").lower():
                        matched.add(pname)

            elif ctype == "project_code":
                # 匹配项目编号（从元数据中取）
                for pname in all_projects:
                    meta = self.get_project_metadata(pname) or {}
                    pcode = str(meta.get("project_code", "") or "")
                    if cval.lower() in pcode.lower():
                        matched.add(pname)

            elif ctype == "project_date":
                # 优先匹配 Milvus 字段中的日期
                for pname, dval in project_dates.items():
                    if dval and cval in str(dval):
                        matched.add(pname)
                # 回退检查元数据中的日期字段
                for pname in all_projects:
                    if pname in matched:
                        continue
                    meta = self.get_project_metadata(pname) or {}
                    sd = meta.get("start_date") or meta.get("project_date") or ""
                    if sd and cval in str(sd):
                        matched.add(pname)
                ...
                for pname in all_projects:
                    if cval.lower() in (pname or "").lower():
                        ...
                        continue
                    meta = self.get_project_metadata(pname) or {}
                    if cval.lower() in str(meta.get("project_code", "")).lower():
                        matched.add(pname)

        logger.info(f"[筛选计算] 当前约束数: {len(constraints)} -> 匹配项目数: {len(matched)}")
    ...
    def remove_constraint(self, index: int):
        ...
        try:
            if 0 <= index < len(constraints):
                ...
                logger.info(f"[上下文更新] 已移除约束: {removed}")
        except Exception as e:
            logger.warning(f"移除约束失败: {str(e)}")

        # 重新计算
        self.context_manager["current_project_set"] = self._compute_project_set_for_constraints()
        return self._get_context_info()

    def clear_context(self):
        """清空所有筛选约束与激活项目集合。"""
        self.context_manager["constraints"] = []
        self.context_manager["current_project_set"] = []
        logger.info("[上下文清理] 已清空所有筛选上下文")
        return self._get_context_info()

    def get_current_project_set(self):
        """返回当前激活的项目集合；若为空则重新计算。"""
        cps = self.context_manager.get("current_project_set")
        if cps is None:
            cps = self._compute_project_set_for_constraints()
            self.context_manager["current_project_set"] = cps
        return cps or []
    ...
    def generate_answer(self, prompt: str, **kwargs) -> str:
        """
        调用 ModelScopeLLM 生成回答。
        prompt 可以是 str / dict，**kwargs 可传 temperature、max_tokens 等。
        """
        import json
        try:
            # 1️⃣ 统一处理输入
            ...
            # 2️⃣ 调用 ModelScopeLLM 的 generate
            return self.llm.generate(safe_prompt, **kwargs)
    ...
    def _initialize(self):
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # 2️⃣ 测试聊天模型（调用 ModelScopeLLM）
                test_text = self.llm.generate("你好", max_tokens=50, temperature=0.7)
    ...
    def _generate_unified_answer(self, query: str, results: List[Dict]) -> str:
        ...
        try:
            return self.llm.generate(prompt, max_tokens=2000, temperature=0.7)
        except Exception as e:
    ...
    def get_all_projects(self) -> List[str]:
        ...
        try:
            ...
            project_names = {item.get("project_name") for item in res if item.get("project_name")}
            project_names = list(project_names)

            # 同步更新缓存（可选）
            if project_names:
                for name in project_names:
                    if name not in self.project_metadata:
                        self.project_metadata[name] = {"source": "milvus", "cached_at": datetime.now().isoformat()}

            logger.info(f"从Zilliz加载 {len(project_names)} 个项目")
    ...
    def _llm_parse_time_range(self, question: str) -> tuple[Optional[str], Optional[str]]:
        ...
        try:
            generated = self.llm.generate(prompt, max_tokens=200, temperature=0.3)
    ...
    def _get_context_info(self):
        """返回当前上下文信息（供前端显示）"""
        return {
            "current_project_set": list(self.context_manager.get("current_project_set") or []),
            "constraints": list(self.context_manager.get("constraints") or []),
            "session_id": self.context_manager.get("session_id")
        }
    ...
    def delete_projects(self, project_names: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": True,
                "message": f"成功删除 {len(project_names)} 个项目，共 {count_result} 条记录",
            ...
            for project in all_projects:
                ...
                try:
                    res = self.collection.query(
                        expr=expr,
                        output_fields=["project_name"],
                        limit=100000  # 足够大，确保统计完整
                    )
                    count = len(res)
                except Exception as e:
                    logger.warning(f"统计项目 {project} 文档数失败: {str(e)}")
                    count = 0
                ...
                project_stats[project] = {
                    ...
                    "file_count": metadata.get("file_count", count),
                    "last_updated": metadata.get("last_updated", "未知"),
                    "start_date": metadata.get("start_date", "未知")
                }

            logger.info(f"项目统计信息加载完成，共 {len(project_stats)} 项目")
            return project_stats
